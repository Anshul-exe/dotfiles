# Generated by Powerlevel10k configuration wizard on 2025-07-17 at 11:02 IST.
...
() {
  ...
  #################[ rust_version: rustc version (https://www.rust-lang.org) ]##################
  # Rust version color.
  typeset -g POWERLEVEL9K_RUST_VERSION_FOREGROUND=202
...
# Language icon segment based on current directory
function prompt_lang_icon() {
  emulate -L zsh
  ...
  if [[ -f go.mod ]] || ls *.go &>/dev/null; then
    ...
    icon=''   # Node.js
    color=70   # green-ish
  elif [[ -f pyproject.toml ]] || [[ -f requirements.txt ]] || ls *.py &>/dev/null; then
    icon=''   # Python
    color=220  # yellow-ish
  elif [[ -f Cargo.toml ]] || ls *.rs &>/dev/null; then
    icon=''   # Rust
    color=208  # orange-ish
  elif ls *.java &>/dev/null; then
    icon=''   # Java
    color=160
  else
    # No known language here, don't show anything
    return
  fi
