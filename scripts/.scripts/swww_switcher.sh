...
# Apply the selected wallpaper using swww with transition effect
if [ -n "$selected_file" ]; then
  swww img "$selected_file" --transition-type=grow --transition-duration=2 --transition-fps=144 --transition-pos=top-right
else
